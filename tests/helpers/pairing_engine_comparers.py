...
class PairingEngineComparer(ABC):
    ...
    def __call__(self, name: str, number_of_players: int, x_section: XSection, seed: int) -> None:
        """
        Compare the pairing engine clients.

        A tournament with the given specifications is simulated and the pairings generated by the clients for each round
...
class RandomResultsComparerWithShuffle(RandomResultsComparer):
    ...
    Tournaments are simulated by randomly assigning a result to each game. The possible results are a win for white, a
    draw or a win for black with each occurring with equal likelihood. Additionally, the player section of the
    tournament is shuffled at random beforehand.
    """
