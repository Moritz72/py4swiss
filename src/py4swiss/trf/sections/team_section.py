...
class TeamSection(BaseModel):
    ...
    def set_code(self, line: str) -> None:
        ...
        try:
            ...
            raise LineError(f"Invalid team code '{section}'", column=1) from e
        ...
        for i in range(36, len(line), 5):
            try:
                ...
                if bool(line[i:].strip()):
                    raise LineError(f"Incomplete team player starting rank '{line[i:]}'", column=i + 1) from e
                return
            ...
            try:
                ...
                if starting_rank is None:
                    ...
                    raise LineError("Missing team player starting rank", column=i - 4)
                else:
                    self.team_players.append(starting_rank)
            except ValueError as e:
                raise LineError(f"Invalid team player starting rank '{section}'", column=i + 1) from e
