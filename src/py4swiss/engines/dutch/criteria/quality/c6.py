...
class C6(QualityCriterion):
    ...
    "minimize the PSD (This basically means: maximize the number of paired MDP(s); and, as far as
    ...
    def get_shift(cls, bracket: Bracket) -> int:
        ...
        """
        # Since each occurrence of a score difference will be contained in at most one weight, the
        # number of bits needed to represent all such occurrences will always be greater than the
        # sum of all weights in a round pairing.
    ...
    def get_weight(cls, player_1: Player, player_2: Player, zero: DynamicUint, bracket: Bracket) -> DynamicUint:
        ...
        difference of their scores to the minimum score in the given bracket. However, if one of
        the given players is neither an MDP nor a resident, then a weight of 0 will be returned.
        """
        ...
        # The weight counts the number of score differences caused by downfloats that will not
        # occur as well as a malus for the score difference that does occur, if this pair is
        # selected in separate parts of the weight based on the absolute value of the given
        # difference. Thus, with this choice of weight, the maximum round pairing weight sum will
        # minimize the PSD. Note that the weight will always be positive, since the score bracket
        # bits are increasing as a function of the score difference.
