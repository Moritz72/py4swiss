...
class C5(QualityCriterion):
    ...
    def get_shift(cls, bracket: Bracket) -> int:
        ...
        # Since the weight for each pair will be at most 1, the number of residents in the bracket
    ...
    def get_weight(cls, player_1: Player, player_2: Player, zero: DynamicUint, bracket: Bracket) -> DynamicUint:
        """Returns a weight of 1, if both players are either MDPs or residents, else 0."""
        weight = DynamicUint(zero)

        # Only pairings between MDPs or residents count as pairs. Thus, with this choice of weight,
        # the maximum round pairing weight sum will maximize the number of pairs.
