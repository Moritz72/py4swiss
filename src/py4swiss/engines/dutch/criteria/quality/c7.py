...
class C7(QualityCriterion):
    ...
    downfloaters in order first to maximize the number of pairs and then to minimize the PSD (see
    C.5 and C.6) in the following bracket (just in the following bracket)."
    """

    @classmethod
    def get_shift(cls, bracket: Bracket) -> int:
        """
        Returns the number of bits needed to represent all lower residents plus the number of bits
        needed to represent all occurrences of all score differences between MDPs and residents in
        the given bracket.
        """
    ...
    def get_weight(cls, player_1: Player, player_2: Player, zero: DynamicUint, bracket: Bracket) -> DynamicUint:
        ...
        difference of their scores to the minimum score in the given bracket as well as whether
        the given players are lower residents or not.
        """
        ...
        # Only pairings with lower residents count as pairs. Thus, with this choice of weight,
        # the maximum round pairing weight sum, will maximize the pairs.
        weight |= int(player_2.role == PlayerRole.LOWER)

        weight <<= bracket.score_difference_total_bits

        # As the scores of all lower residents is the same, if the given bracket is not the PPB,
        # and the scores of potential pairs with MDPs and residents is already determined by C.6,
        # it is sufficient to only handle double floats of MDPs and residents, see C.6 for
        # comparison.
