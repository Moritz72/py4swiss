...
class C12(QualityCriterion):
    ...
    def get_shift(cls, bracket: Bracket) -> int:
        ...
        if not bracket.one_round_played:
            return 0
        # Since the weight for each pair will be at most 2, the number of residents in the bracket
        # will always be greater than the sum of all weights.
    ...
    def get_weight(cls, player_1: Player, player_2: Player, zero: DynamicUint, bracket: Bracket) -> DynamicUint:
        ...
        downfloat in the previous round but will not receive one this round, assuming the players
        are paired with each other. However, if one of the given players is neither an MDP nor a
        ...
        # The lower ranked player has the same as or fewer points than the higher ranked player.
        # Thus, the former will never receive a downfloat when paired with a higher ranked player.
        # Note also that unpaired players will be MDPs in the following bracket or receive the
        # pairing-allocated bye and thus downfloat. Thus, it is necessary to count exactly the
        # number of residents in given bracket who received a downloat in the previous round, but
        # will not receive one in this round, in order to get the maximum round pairing weight sum
        # to minimize the number of paired players that receive the same downfloat as the previous
        # round.
