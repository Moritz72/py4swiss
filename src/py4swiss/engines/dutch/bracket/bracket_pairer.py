...
class BracketPairer:
    ...
    def __init__(self, state: Bracket, validity_matcher: ValidityMatcher) -> None:
        """
        Initializes the `BracketPairer`.

        The `BracketPairer` initializes a bracket matcher along with the heterogeneous and
    ...
    def _get_player_pair(player_1: Player, player_2: Player) -> tuple[Player, Player]:
        """
        Return a tuple of the given players where the first player in the tuple is to receive the
        white pieces and the second player the black pieces in adherence to the color criteria.
        """
        ...
        # Evaluate the color criteria E.1 - E.5 in order until one is conclusive. Note that E.5 is
        # always conclusive.
    ...
    def _in_s1(self, player: Player) -> bool:
        """
        Check if the given player is currently considered to be in S1 i.e. matched with a player
        ranked below them.
        """
    ...
    def _in_s2(self, player: Player) -> bool:
        """
        Check if the given player is currently considered to be in S2 i.e. matched with a player
        ranked above them or not matched at all.
        """
    ...
    def determine_heterogeneous_s1(self) -> None:
        ...
        # Since the quality criteria are already taken into account by the bracket matcher, only
        # the transpositions need to be accounted for, specifically the exchanges from S1 to Limbo
        # (D.3).

        # "An exchange in a heterogeneous bracket (also called a MDP-exchange) is a swap of two
        # equally sized groups of BSN(s) (all representing MDP(s)) between the original S1 and the
        # original Limbo.
        ...
        # As also mentioned in the rules document, D.3.a is already taken care of by C.6, thus only
        # D.3.b needs to be considered.
        for mdp in self._bracket.mdp_list:
            # If the MDP is currently not unpaired, incentivize pairing it by adding 1 to each
            # edge weight between it and each resident. This will force the MDP to be paired in
            # the new matching, if this is possible while still satisfying all quality criteria
            # as well as before this modification.
            ...
            # If the MDP is paired after the optional modification, finalize the fact that it is
            # in S1 by further incentivizing its pairings such that later modifications will not
            # overwrite this choice. Since the list of MDPs is ordered by BSN, this ensures that
            # D.3.b is adhered to.
    ...
    def determine_heterogeneous_s2(self) -> None:
        """Determine the order of S2 in the heterogeneous bracket."""
        # Since the quality criteria are already taken into account by the bracket matcher, only
        # the transpositions need to be accounted for, specifically D.1.
        ...
        # first N1 BSN(s), where N1 is the number of BSN(s) in S1 (the remaining BSN(s) of S2 are
        # ignored in this context, because they represent players bound to constitute the remainder
        # in case of a heterogeneous bracket; or bound to downfloat in case of a homogeneous
        # bracket - e.g. in a 11-player homogeneous bracket, it is 6-7-8-9-10, 6-7-8-9-11,
        # 6-7-8-10-11, ..., 6-11-10-9-8, 7-6-8-9-10, ..., 11-10-9-8-7 (720 transpositions); if the
        # bracket is heterogeneous with two MDPs, it is: 3-4, 3-5, 3-6, ..., 3-11, 4-3, 4-5, ...,
        # 11-10 (72 transpositions))."
        for mdp in self._heterogeneous_s1:
            # Incentivize the MDP being paired with the resident players. Pairing with the highest
            # ranked resident is incentivized the most, while pairing with the lowest is
            # incentivized the least. This ensures that the MDP is paired with the resident that
            # has the lowest BSN out of all possible choices.
            ...
            # Finalize the pairing of the MDP and the chosen resident so that it does not get
            # overwritten in the future. This ensures that MDPs with lower BSN take precedence
            # such that D.1 is fully adhered to.
    ...
    def determine_homogeneous_exchanges(self) -> None:
        ...
        # This step is technically not necessary. It is possible to determine the composition of S1
        # in one step. However, this requires to recompute the optimal pairings per resident,
        # similar to the heterogeneous case. In contrast, determining the number of exchanges can
        # be done at the cost of only one recomputation and improve the efficient of later steps.
        # The number of exchanges is determined by D.2, specifically D.2.a.

        # "An exchange in a homogeneous brackets (also called a resident-exchange) is a swap of two
        # equally sized groups of BSN(s) (all representing resident players) between the original
        # S1 and the original S2.
        ...
        #    and the sum of the BSN(s) moved from the original S1 to S2 (e.g. in a bracket
        #    containing eleven players, exchanging 6 with 4 is better than exchanging 8 with 5;
        #    similarly exchanging 8+6 with 4+3 is better than exchanging 9+8 with 5+4; and so on).
        # c. the highest different BSN among those moved from the original S1 to S2 (e.g. moving 5
        #    from S1 to S2 is better than moving 4; similarly, 5-2 is better than 4-3; 5-4-1 is
        #    better than 5-3-2; and so on).
        # d. the lowest different BSN among those moved from the original S2 to S1 (e.g. moving 6
        #    from S2 to S1 is better than moving 7; similarly, 6-9 is better than 7-8; 6-7-10 is
        #    better than 6-8-9; and so on)."

        # Determine how many pairs can be formed after excluding the previously paired
        ...
        # Incentivize pairings according to D.2.a and D.2.b. D.2.b is not strictly necessary for
        # determining the number of exchanges, but since it would need to be done later on anyway,
        # it is taken care of in this step already.
        for i, resident in enumerate(remainder):
            # Incentivize pairs containing players in S1. This will in sum maximize the number
            # of pairs between S1 and S2 and thus, minimize the number of exchanges taking care of
            # D.2.a. For D.2.b, a pairing is disincentivized by a value equal to the minimum BSN of
            # the pair. Thus, in sum, minimizing the difference between the sum of BSNs moved from
            # S1 to S2 and the sum of BSNs moved from S2 to S1, since, when pairing is completed, a
            # player is in S1, if and only if they are paired with a resident with a higher BSN and
            # the number of exchanges is already fixed by D.2.a. Note that the sum of the additions
            # to the weights needed for D.2.b is always smaller than the square of the number of
            # residents. Thus, a shift of twice the number of bracket bits is sufficient to avoid
            # interference with the additions for D.2.a.
    ...
    def determine_moves_from_s1_to_s2(self) -> None:
        ...
        # Since D.2.a and D.2.b are already accounted for, it is only necessary to determine the
        # highest different BSN among those moved from the original S1 to S2 (D.2.c). This step
        # is performed in a similar fashion as the choice of S1 in the heterogeneous case.
        for i in range(len(self._homogeneous_s1) - 1, -1, -1):
            ...
            # If the resident is currently in S2, disincentivize pairings that would keep the
            # resident in S2, i.e. ones with lower ranked residents, by subtracting 1 to each edge
            # weight between them. This will force the resident to be in S1, if this is possible
            # while still satisfying all quality criteria as well as before this modification.
            if not was_exchanged:
                self._bracket_matcher.add_to_weights(resident, lower_residents, -1)
                self._bracket_matcher.update_matching()

            # If the resident remains in S2 even after the optional modification, finalize the fact
            # that it will be exchanged by removing all edge weights with lower ranked residents.
            # Since the current S1 is ordered by BSN, this ensures that D.2.c is adhered to.
    ...
    def determine_moves_from_s2_to_s1(self) -> None:
        ...
        # Since D.2.a and D.2.b are already accounted for, it is only necessary to determine the
        # lowest different BSN among those moved from the original S2 to S1 (D.2.d). This step
        # is performed in the exact same way as the moves from S1 to S2.
        for i, resident in enumerate(self._homogeneous_s2):
            ...
            # If the resident is currently in S1, incentivize pairings that would move the resident
            # into S2, i.e. ones with lower ranked residents, by subtracting 1 to each edge weight
            # between them. This will force the resident to be in S2, if this is possible while
            # still satisfying all quality criteria as well as before this modification.
            if not was_exchanged:
                self._bracket_matcher.add_to_weights(resident, higher_residents, 1)
                self._bracket_matcher.update_matching()

            # If the resident remains in S1 even after the optional modification, finalize the fact
            # that it will be exchanged by removing all edge weights with higher ranked residents.
            # Since the current S2 is ordered by BSN, this ensures that D.2.d is adhered to.
            if self._in_s2(resident):
                self._exchanges -= 1
    ...
    def transpose_homogeneous_s2(self) -> None:
        ...
        for resident in self._homogeneous_s1:
            # Incentivize the player in S1 being paired with the players in S2 in order. Pairing
            # with the highest ranked player in S2 is incentivized the most, while pairing with the
            # lowest is incentivized the least. This ensures that the player in S1 is paired with
            # the player in S2 that has the lowest BSN out of all possible choices.
            ...
            # Finalize the pairing of the player in S1 and the chosen player in S2 so that it does
            # not get overwritten in the future. This ensures that players in S1 with lower BSN
            # take precedence such that D.1 is fully adhered to.
    ...
    def check_completion_criterium(self) -> bool:
        """
        Check whether it is possible to complete the round pairing with the chosen set of
        downfloaters.
        """
    ...
    def get_player_pairs(self) -> list[tuple[Player, Player]]:
        """
        Return the chosen pairings as a list of tuples where the first player in each tuple is to
        receive the white pieces and the second player the black pieces in adherence to the color
        criteria. A possibly unpaired player is denoted by a tuple of that player with themselves.
        """
        player_pairs = []
        ...
        for player_1, player_2 in self._bracket_matcher.matching.items():
            ...
            # If the current bracket is the LPB, there might be one unpaired player left in the
            # bracket. This player will consequently receive the pairing-allocated bye.
